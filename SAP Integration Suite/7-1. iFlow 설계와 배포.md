...
S/4HANA OData를 직접 호출하는 방식과, 이 iFlow처럼 Integration Suite를 중간에 두는 방식의 역할 차이는 [8. S/4HANA OData 직접 연계와 Integration Suite 비교](8.%20S4HANA%20OData%20%EC%A7%81%EC%A0%91%20%EC%97%B0%EA%B3%84%EC%99%80%20Integration%20Suite%20%EB%B9%84%EA%B5%90.md)를 참고한다.

```text
호출자(JSON)
  → HTTPS Sender
  → JSON to XML Converter
  → Content Modifier (상품 ID를 헤더에 저장)
  → Request Reply
  → OData V2 Receiver
  → 호출자에게 응답
```
