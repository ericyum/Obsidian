# 7-2. API Management로 iFlow API 노출과 테스트
...
```text
API 소비자
  → API Proxy (/products/details)
  → OAuth 정책이 access token 획득
  → 배포된 iFlow endpoint
  → 외부 OData 서비스
```

> 이 문서는 [7-1. iFlow 설계와 배포](7-1.%20iFlow%20설계와%20배포.md)에서 iFlow가 `Deployed` 및 `Started` 상태인 것을 전제로 한다.
