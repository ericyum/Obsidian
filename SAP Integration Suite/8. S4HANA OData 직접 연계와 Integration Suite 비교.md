...
1. 프론트엔드가 S/4HANA 주문 API, CRM 고객 API, 배송 SaaS API를 각각 호출한다. 또는 별도 Node.js/Java BFF(backend for frontend)를 새로 만든다.
2. 프론트엔드/BFF가 세 응답의 필드명·형식·오류·timeout을 맞추고 하나의 화면 DTO로 조합한다.
3. 세 시스템 중 하나의 API 형식·인증·주소가 바뀌면 BFF 또는 프론트엔드를 수정·배포한다.

#### IS를 넣으면

1. 소비자는 예를 들어 `/order-summary/{id}`라는 하나의 API만 호출한다.
2. Cloud Integration 또는 API-centric Integration이 S/4HANA, CRM, 배송 시스템을 호출한다.
3. Mapping/Converter로 응답 형식을 맞추고, Content Enricher 또는 Request Reply로 필요한 데이터를 추가·조합한다.
4. API Management가 소비자에게 보이는 경로와 보안 정책을 관리한다.

#### 얻는 이점

- 프론트엔드는 SAP OData·CRM REST·배송 SaaS의 내부 차이를 알 필요가 없다.
- backend 하나의 URL·필드명이 바뀌어도, 외부 계약을 유지할 수 있는 변경 지점이 IS에 모인다.
- 새 소비자가 생겨도 같은 조합 API를 재사용한다.

> SAP는 API-centric Integration의 사용 사례로 **여러 backend 데이터 집계, payload 변환·매핑, 순차/조건부 서비스 호출, 요청 검증·enrichment**를 제시한다. 이 시나리오는 IS가 분명한 가치를 주는 대표 사례다.
...
1. S/4HANA OData 모델을 거래처가 이해하도록 직접 공개하거나, 별도 커스텀 프로그램을 만든다.
2. 거래처별 JSON, CSV, EDI, AS2/SFTP, 인증서, 오류 응답을 각자 구현·운영한다.
3. 거래처가 늘 때마다 S/4HANA 쪽 또는 별도 애플리케이션에 새 연계를 추가한다.

#### IS를 넣으면

1. S/4HANA의 주문 데이터를 OData, IDoc, RFC 등 적합한 방식으로 수신/조회한다.
2. 거래처가 요구하는 형식과 프로토콜로 Mapping·Converter·adapter를 적용한다.
3. Router로 거래처별 endpoint를 분기하고, 공통 오류 처리·추적 방식을 iFlow에 둔다.
4. 외부에 HTTP API를 제공해야 하면 API Management로 인증·접근 정책을 별도로 적용한다.

#### 얻는 이점

S/4HANA 내부 데이터 모델을 거래처별 계약에 맞추어 직접 노출하지 않고, **S/4HANA 변경과 거래처 인터페이스 변경의 완충층**을 둔다. SAP FSD는 Cloud Integration이 adapter와 처리 단계로 sender/receiver를 연결하며, Mapping·Converter·Content Modifier·Request-Reply를 제공한다고 설명한다.
...
1. S/4HANA 또는 자체 BTP backend endpoint를 외부 클라이언트가 호출한다.
2. 각 API 또는 backend에서 OAuth, 권한, IP 제한, 요청 검증, 호출량 제한, 로그·분석을 각각 구현하거나 별도 gateway를 운영한다.

#### IS를 넣으면

1. API Management의 API Proxy가 S/4HANA/backend 앞의 관리형 진입점이 된다.
2. Proxy에 OAuth·인증·인가, IP filtering, JSON/XML 위협 방지, API validation 같은 정책을 적용한다.
3. quota·surge protection 등 트래픽 제어와 usage/성능 분석을 API 수준에서 관리한다.
4. 소비자는 backend의 실제 주소·내부 변경 대신 안정적인 공개 API 계약을 사용한다.

#### 얻는 이점

기존 CDS/BTP/S/4HANA 인증이 이미 있어도, **여러 공개 API에 공통 정책을 코드로 반복 구현하지 않고 API 계층에서 적용**할 수 있다. SAP는 API Proxy가 backend 변경과 API 소비자를 분리하고, 인증·인가·rate limiting·throttling·요청/응답 변환·분석을 수행할 수 있다고 설명한다.

> 이 시나리오에서 필요한 것이 보호·거버넌스뿐이면 **API Management만**으로 시작할 수 있다. 변환·여러 시스템 호출까지 필요할 때 Cloud Integration/API-centric Integration을 함께 검토한다.
...
1. 클라우드 애플리케이션이 온프레미스 S/4HANA endpoint에 도달할 네트워크 경로를 설계한다.
2. 방화벽, 인증서, 접근 권한, endpoint 노출 범위를 애플리케이션별로 관리한다.
3. SaaS와 S/4HANA의 프로토콜·형식 차이가 있으면 각 연계 애플리케이션에서 처리한다.

#### IS를 넣으면

1. Cloud Integration의 adapter를 sender/receiver에 구성하고, cloud-to-on-premise 시나리오를 iFlow로 설계한다.
2. 연결 방식·보안 설정·변환을 통합 artifact에 모은다.
3. 동일한 S/4HANA 연결을 여러 통합 시나리오에서 재사용할 수 있도록 설계한다.

#### 얻는 이점

애플리케이션마다 온프레미스 연결 로직을 중복하기보다, 통합 계층에서 연결·메시지 처리를 표준화한다. 단, Cloud Connector, 방화벽, 인증서, S/4 권한 설계 자체가 사라지는 것은 아니다.
...
1. 각 소비 시스템이 일정 주기마다 S/4HANA OData를 조회한다.
2. 갱신 전까지 데이터가 늦을 수 있고, 소비자가 늘면 조회 부하와 polling 구현이 늘어난다.
3. 소비 시스템이 일시적으로 중단되었을 때 누락·재처리 방식을 각자 설계해야 한다.

#### IS의 Event Mesh를 쓰면

1. S/4HANA 등의 event source가 업무 이벤트를 publish한다.
2. 여러 소비자가 Event Mesh에서 각자 구독·처리한다.
3. 새 소비자는 기존 S/4HANA OData API를 바꾸지 않고 구독을 추가한다.

#### 얻는 이점

이것은 OData 직접 호출을 “더 잘하는” 방식이 아니라, **조회 중심 요구를 이벤트 중심 요구로 바꾸는 경우의 다른 통합 패턴**이다. SAP Event Mesh는 애플리케이션 간 business event의 publish·consume을 지원한다.
...
Fiori/BFF 로그, S/4HANA 로그, SaaS 로그, API Gateway 로그를 각각 확인해 하나의 업무 요청이 어디에서 실패했는지 연결해야 한다.

#### IS를 넣으면

iFlow와 API Proxy가 통과 지점이 되므로 Cloud Integration과 API Management Monitor에서 통합·API 관점의 실행·성능 정보를 볼 수 있다.

#### 얻는 이점과 한계

통합 흐름의 운영 지점은 중앙화되지만, S/4HANA 내부 업무 오류의 원인 분석까지 IS가 대체하는 것은 아니다. 상관관계 ID, 오류 메시지 기준, 재처리 기준은 여전히 설계해야 한다.
