# zen-ai 기존 Agent Framework 기반 회의록 Agent 구현 전략
...
> 이 문서는 그 설계 자산을 현재 `zen-ai` 구조에 어떤 형식으로 구현할지를 정리한다.
...
Agent 정의는 `agents`와 `agent_versions`에 저장된다. 사용자에게 Agent가 부여되어 있고 메시지에서 해당 Agent를 호출하면, 현재 버전과 사용자별 유효 설정을 바탕으로 실행된다.

회의록 Agent 역시 새로운 독립 서버가 아니라 다음과 같은 제품 Agent 정의로 등록한다.
...
현재 외부 Tool은 MCP 연결을 통해 동적으로 구성할 수 있다. 그러나 회의록 데이터는 같은 서비스 내부의 DB와 권한 모듈에 존재하므로, 회의록 Tool은 우선 **서버 내부 ToolSet**으로 구현하는 것이 적절하다.
...
`worker.ts` 전체를 회의록 전용으로 복사하지 않는다. 대신 Agent `slug`나 명시적인 Runtime 종류를 기준으로 전용 Orchestrator를 선택할 수 있는 작은 확장 지점을 둔다.
...
Search Tool은 인증 사용자가 접근할 수 있는 회의록만 검색해 안전한 후보 메타데이터를 반환한다.
...
Tool 2는 모델이 ID를 선택하는 형태가 아니라, 서버가 현재 Session의 허용 ID를 Closure에 결합하는 형태로 구성한다.
...
Agent의 자연어 응답만으로 브라우저 모달을 직접 열 수는 없다. 향후 자연어 녹화 명령까지 지원하려면 Agent가 구조화된 UI Command Event를 발행하고, 현재 접속한 사용자 Client가 이를 받아 기존 Provider 함수를 호출하는 Bridge가 필요하다.
