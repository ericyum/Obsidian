...
**설계 의도**
 ...
 - 회의록 활용 과정에서는 **질문 대상이 되는 회의록을 찾는 상태**와 그렇지 않고 **현재 상태에서 질문을 하는 상태**로 분류 할 수 있다고 판단하여, `search`와 `none`의 2가지 상태로 분류한다.
 - 즉, 모든 검색 과정은 `search` 내부에서 처리하며, 회의록 질문과 답변은 `none` 상태에서 처리한다.
...
![[assets/회의-녹화-Agent-Graph.svg]]

<details>
<summary>Mermaid 원본 보기</summary>

```mermaid
%%{init: {"flowchart": {"defaultRenderer": "elk", "useMaxWidth": true, "nodeSpacing": 60, "rankSpacing": 70, "curve": "linear"}, "elk": {"mergeEdges": false, "nodePlacementStrategy": "LINEAR_SEGMENTS", "keepEntryNodeOnTop": true}, "themeVariables": {"fontSize": "11px"}}}%%
flowchart TB
    ...
    A -.- RA["💬 회의 시작 실패 후 재시도<br/>RETRYABLE_FAILED"]
    B -.- RB["💬 회의 제어 실패 후 재시도<br/>RETRYABLE_FAILED"]
    C -.- RC["💬 확정·공유 실패 후 재시도<br/>RETRYABLE_FAILED"]
    ...
    class RA,RB,RC note;
    linkStyle default stroke:#7a7a7a,stroke-width:1.5px;
    linkStyle 5,6,7 stroke:#f97316,stroke-width:1.5px,stroke-dasharray: 5 4;
```
...
![[assets/회의록-검색-Agent-Graph-v2.svg]]

<details>
<summary>Mermaid 원본 보기</summary>

```mermaid
%%{init: {"flowchart": {"defaultRenderer": "elk", "useMaxWidth": true, "nodeSpacing": 90, "rankSpacing": 110, "curve": "stepAfter"}, "elk": {"mergeEdges": false, "nodePlacementStrategy": "NETWORK_SIMPLEX", "nodePlacementAlignment": "BALANCED", "cycleBreakingStrategy": "MODEL_ORDER", "forceNodeModelOrder": true, "considerModelOrder": "NODES_AND_EDGES", "keepEntryNodeOnTop": true}, "themeVariables": {"fontSize": "11px"}}}%%
flowchart TB
    S(["START"])
    subgraph SEARCH_ROW[" "]
        direction LR
        RA["💬 검색 실패 후 재시도<br/>RETRYABLE_FAILED"] -.- A["1. 회의록 검색 Node<br/>LLM 검색 조건 구성<br/>Tool 2 권한 필터·후보 검색"]
        A -->|"후보 없음·활성 ID 없음<br/>NO_CANDIDATES"| E0(["END<br/>후보 없음"])
    end
    subgraph SELECT_ROW[" "]
        direction LR
        G["2. 회의록 선택 Gate<br/>1개 자동 선택<br/>2개 이상 Human in the Loop<br/>Tool 3 ID 검증"] -->|"선택 취소<br/>CANCELLED"| E1(["END<br/>선택 취소"])
    end
    subgraph QUERY_ROW[" "]
        direction LR
        RQ["💬 질의 실패 후 재시도<br/>RETRYABLE_FAILED"] -.- Q["3. 회의록 질의 Node<br/>Tool 4 원문 조회<br/>컨텍스트 조립 + LLM 답변"]
        Q -->|"질문 1회 처리 완료<br/>QUESTION_HANDLED"| E2(["END<br/>이번 Graph 실행 완료<br/>Agent는 none에서 입력 대기"])
    end
    S --> A
    A -->|"후보 있음<br/>CANDIDATES_FOUND"| G
    A -->|"후보 없음·기존 ID 유지<br/>NO_CANDIDATES"| Q
    ...
    class RA,RQ note;
    style SEARCH_ROW fill:transparent,stroke:transparent;
    style SELECT_ROW fill:transparent,stroke:transparent;
    style QUERY_ROW fill:transparent,stroke:transparent;
    linkStyle default stroke:#7a7a7a,stroke-width:1.5px;
    linkStyle 1,2,7,9 stroke:#d93025,stroke-width:2px;
    linkStyle 0,3 stroke:#f97316,stroke-width:1.5px,stroke-dasharray: 5 4;
```

</details>

이 Agent Graph의 Node는 검색, 선택, 질의처럼 독립적으로 실행·검증할 수 있는 작업 단위다. Edge는 후보 존재 여부, ID 확정, 질문 1회 처리 완료처럼 다음 작업 또는 이번 Graph 실행의 종료를 허용하는 구조화된 Outcome을 나타낸다. 각 Node는 Outcome과 함께 필요한 State 업데이트를 반환하지만, State 갱신과 Checkpoint 저장 자체를 상위 Graph의 별도 작업 Node로 만들지는 않는다. 검색과 질문의 구체적인 내부 흐름은 아래 4-1과 4-2 순서도에서 나누어 설명한다.

그림의 세 `END`는 서로 다른 Agent나 State가 아니라, 후보 없음·선택 취소·질문 1회 처리 완료라는 서로 다른 사유로 이번 Graph 실행이 끝나는 출구를 각각 가까운 위치에 표시한 것이다. `QUESTION_HANDLED` 뒤의 `END`는 에이전트 세션이나 질의 과정의 종료가 아니며, 에이전트는 `none`에서 다음 사용자 입력을 계속 기다린다.
...
- **회의록 질의 Node**: 확정된 ID로 원문을 조회하고 컨텍스트를 조립해 질문 한 번에 답변하는 과정을 하나의 실행 단위로 묶었다. 답변을 반환하면 이번 Graph 실행은 끝나지만 에이전트는 `none`에서 다음 입력을 기다리며, 다음 질문이 들어오면 같은 질의 Node가 새 Graph 실행에서 다시 사용된다. 회의록을 추가하거나 대체하는 입력이면 검색 Node로 이동한다.
- **START·END**: 업무를 수행하는 Node가 아니라 Graph의 진입점과 종료점을 나타내는 제어용 Terminal이다.
...
| Tool 2 | 관련 회의록 후보 검색 | `none` | 검색 진입 시 `search` |
| Tool 3 | 선택한 회의록 ID 검증·반영 | `search` | 추가·대체 반영 후 `none` |
| Tool 4 | 허용된 회의록 원문 조회 | `none` 및 ID 허용 목록 포함 | 질문·답변 중 `none` 유지 |
...
- 회의록 질문은 `none`에서 처리하며, 회의록을 추가하거나 대체하려면 `none`에서 다시 `search`로 진입한다. 기존 회의록 ID가 있으면 Human in the Loop로 새 ID의 추가·대체를 선택한 뒤 `none`으로 돌아가 질의응답을 계속한다.
