...
이 문서는 [[1. 회의록 에이전트 워크플로우 graph 정의]]와 [[2. 회의록 에이전트 설계]]를 기준으로, 대화 기록과 선택된 회의록 원문을 LLM에 어떤 방식으로 전달할지를 정의한다.
...
회의록 에이전트에서는 컨텍스트 조립에 사용하는 정보를 다음 네 영역으로 나누어 관리한다. State와 Checkpoint의 전체 구조와 저장 기준은 [[2. 회의록 에이전트 설계]]에서 다룬다.

| 영역 | 관리하는 내용 | 유지 범위 | LLM이 직접 보는가? |
|---|---|---|---|
| State의 `messages` | 사용자 요청, LLM 답변, Tool 호출과 결과, HITL 입력·선택 결과 | 사용자별 전용 스레드 | 모델 호출에 포함한 부분만 봄 |
| Runtime Context | 인증된 사용자, 현재 전용 스레드, 기존 백엔드·DB 접근 수단 | 현재 실행 | 자동으로 보지 못함 |
| 실행 중 문서 데이터 | Tool 6이 회의록 ID로 가져온 원문 하나 이상 | 현재 LLM 호출 준비 과정 | Model Context에 넣을 때만 봄 |
...
State와 `workflow_state`의 정의, 사용자별 전용 스레드 분리 기준은 [[2. 회의록 에이전트 설계]]에서 다룬다. 이 문서에서는 State 전체가 아니라 Model Context의 대화 입력으로 사용하는 `messages`만 다룬다.
...
- Tool 5가 반환한 선택된 회의록 ID 결과

회의록 원문 전체는 `messages`에 저장하지 않는다. Tool 5가 반환한 회의록 ID는 이후 질문에서 같은 회의록 집합을 다시 조회하기 위한 기준값으로 사용한다.
...
Tool 4와 Tool 5의 검색·선택 방식은 [[2. 회의록 에이전트 설계]]에서 다룬다. 컨텍스트 구조에서 Tool 5의 결과인 회의록 ID는 원문을 직접 저장하지 않고 필요할 때 다시 조회하기 위한 문서 참조값이다.

Tool 5의 ID 결과는 해당 전용 스레드의 `messages`에 남는다. 사용자가 질의 대상을 확정한 이후에는 같은 회의록 집합을 바탕으로 연속 질문할 수 있다.

## 6. Tool 6과 실행 중 문서 데이터

Tool 6은 Tool 5가 반환한 ID를 이용해 기존 ZEN AI 백엔드에서 회의록 원문 하나 이상을 가져온다.

```text
Tool 5의 회의록 ID 결과
→ Tool 6이 기존 백엔드에서 원문 조회
→ 원문을 실행 중 문서 데이터로 전달
...
일반적인 Tool 결과처럼 회의록 원문 전체를 ToolMessage로 `messages`에 넣으면 같은 문서가 다시 누적된다. 따라서 Tool 6의 영속적인 Tool 결과에는 조회 성공 여부, 사용한 회의록 ID, 문서 수와 같은 메타데이터만 남길 수 있고, 원문 본문은 오케스트레이터 또는 컨텍스트 조립 단계에 일시적으로 전달해야 한다.
...
flowchart TB
    ...
    S --> ID["Tool 5가 반환한<br/>회의록 ID 결과 확인"]
    ID --> T6["Tool 6<br/>ID로 기존 백엔드 원문 조회"]
    RT["Runtime Context<br/>인증 사용자·백엔드 접근 수단"] --> T6
    T6 --> D["실행 중 문서 데이터<br/>회의록 원문 하나 이상"]
...
2. Agent가 현재 `messages`에서 Tool 5가 반환한 회의록 ID 결과를 확인한다.
3. Tool 6이 해당 ID를 이용해 기존 백엔드에서 회의록 원문 하나 이상을 가져온다.
...
Tool 5가 반환한 현재 질의 대상 회의록 ID는 연속 질문에 필요하다. 따라서 Compact를 구현할 때 활성 회의록 ID 결과가 삭제되거나 모호한 자연어 요약으로만 바뀌지 않도록 보존 방식을 정해야 한다.
...
권한 검사 방식은 [[2. 회의록 에이전트 설계]]에서 다룬다. 컨텍스트 조립 단계에서는 권한 정보 자체를 Model Context에 넣어 LLM이 접근 가능 여부를 판단하게 하지 않고, 백엔드 권한 필터를 통과한 회의록만 입력으로 사용한다.
...
- Tool 6이 원문을 컨텍스트 조립 단계에 전달하는 구체적인 구현 방식
- Tool 6의 영속적인 Tool 결과에 남길 메타데이터 Schema
- Compact 이후에도 활성 회의록 ID를 정확히 보존하는 방법
- 긴 회의록을 전체로 넣을지 관련 구간만 넣을지
- 조립된 회의록 원문을 어떤 Message 역할과 형식으로 모델에 전달할지

## 핵심 정리

> `messages`는 LLM 호출에 사용할 대화와 Tool 결과를 제공하고, 질의 대상으로 확정된 회의록 ID는 원문을 다시 조회하기 위한 참조값으로 사용한다. 질문이 들어오면 Tool 6이 기존 백엔드에서 ID에 해당하는 원문을 가져오고, LLM 호출 시점에만 `시스템 지시 + 회의록 원문 + messages`를 조립한다. 호출 후에는 답변만 `messages`에 추가한다.
