...
> 소스: `app/meeting-ui/js/recorder.js`  
> 역할: MediaRecorder와 Web Audio API를 감싸 녹음 blob과 음량 레벨을 만든다.

## 1. 이 파일이 있는 이유

MediaRecorder와 Web Audio API를 감싸 녹음 blob과 음량 레벨을 만든다.
...
| 항목 | 값 |
|---|---|
| 경로 | `app/meeting-ui/js/recorder.js` |
| 형식 | `javascript` |
| 전체 줄 수 | 103 |
| 주석 줄 수 | 1 |
| 주요 심볼 수 | 2 |
...
| 선언 | 읽을 때 확인할 점 |
|---|---|
| `pickMimeType` | 입력, 상태 변경, 반환값과 실패 경로를 확인한다. |
| `createRecorder` | 입력, 상태 변경, 반환값과 실패 경로를 확인한다. |
