# test_diarize_slow.py 완전 분석

> 소스: `tests/test_diarize_slow.py`  
...
| 선언 | 읽을 때 확인할 점 |
|---|---|
| `test_real_diarization` | 입력, 상태 변경, 반환값과 실패 경로를 확인한다. |

## 5. 환경 변수

- `HF_TOKEN`: 배포 환경과 로컬 실행 값이 달라질 수 있으므로 `mta.yaml`, `.env.example`과 함께 확인한다.

## 7. 코드 흐름 상세

### 7.1 `test_real_diarization`
