# test_memory.py 완전 분석

> 소스: `tests/test_memory.py`  
...
| 선언 | 읽을 때 확인할 점 |
|---|---|
| `test_release_model_memory_collects_gc_without_torch` | 입력, 상태 변경, 반환값과 실패 경로를 확인한다. |
| `test_release_model_memory_clears_cuda_cache` | 입력, 상태 변경, 반환값과 실패 경로를 확인한다. |

## 7. 코드 흐름 상세

### 7.1 `test_release_model_memory_collects_gc_without_torch`

- 위치: 7~17행
- 의미: 이 블록의 전제 조건, 외부 호출, DB/메모리 상태 변경, 예외 처리 순서가 핵심이다.

### 7.2 `test_release_model_memory_clears_cuda_cache`
