# test_storage.py 완전 분석

> 소스: `tests/test_storage.py`  
...
| 선언 | 읽을 때 확인할 점 |
|---|---|
| `test_save_and_load` | 입력, 상태 변경, 반환값과 실패 경로를 확인한다. |
| `test_load_missing_returns_none` | 입력, 상태 변경, 반환값과 실패 경로를 확인한다. |
| `test_list_jobs_and_delete` | 입력, 상태 변경, 반환값과 실패 경로를 확인한다. |

## 7. 코드 흐름 상세

### 7.1 `test_save_and_load`

- 위치: 5~14행
- 의미: 이 블록의 전제 조건, 외부 호출, DB/메모리 상태 변경, 예외 처리 순서가 핵심이다.

### 7.2 `test_load_missing_returns_none`

- 위치: 15~18행
- 의미: 이 블록의 전제 조건, 외부 호출, DB/메모리 상태 변경, 예외 처리 순서가 핵심이다.

### 7.3 `test_list_jobs_and_delete`
