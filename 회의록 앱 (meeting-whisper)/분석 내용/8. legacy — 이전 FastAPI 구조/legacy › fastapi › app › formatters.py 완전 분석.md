...
> 소스: `legacy/fastapi/app/formatters.py`  
...
| 항목 | 값 |
|---|---|
| 경로 | `legacy/fastapi/app/formatters.py` |
| 형식 | `python` |
| 전체 줄 수 | 51 |
| 주석 줄 수 | 0 |
| 주요 심볼 수 | 5 |
...
| 선언 | 읽을 때 확인할 점 |
|---|---|
| `format_timestamp` | 입력, 상태 변경, 반환값과 실패 경로를 확인한다. |
| `_hms` | 입력, 상태 변경, 반환값과 실패 경로를 확인한다. |
| `to_txt` | 입력, 상태 변경, 반환값과 실패 경로를 확인한다. |
| `to_srt` | 입력, 상태 변경, 반환값과 실패 경로를 확인한다. |
| `note_to_markdown` | 입력, 상태 변경, 반환값과 실패 경로를 확인한다. |

## 7. 코드 흐름 상세

### 7.1 `format_timestamp`
