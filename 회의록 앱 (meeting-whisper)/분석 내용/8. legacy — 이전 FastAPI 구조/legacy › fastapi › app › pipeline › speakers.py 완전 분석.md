...
> 소스: `legacy/fastapi/app/pipeline/speakers.py`  
...
| 항목 | 값 |
|---|---|
| 경로 | `legacy/fastapi/app/pipeline/speakers.py` |
| 형식 | `python` |
| 전체 줄 수 | 23 |
| 주석 줄 수 | 0 |
| 주요 심볼 수 | 2 |
...
| 선언 | 읽을 때 확인할 점 |
|---|---|
| `dominant_speaker` | 입력, 상태 변경, 반환값과 실패 경로를 확인한다. |
| `fill_segment_speakers` | 입력, 상태 변경, 반환값과 실패 경로를 확인한다. |

## 7. 코드 흐름 상세

### 7.1 `dominant_speaker`

- 위치: 7~17행
- 의미: 이 블록의 전제 조건, 외부 호출, DB/메모리 상태 변경, 예외 처리 순서가 핵심이다.

### 7.2 `fill_segment_speakers`
