...
> 소스: `legacy/fastapi/app/pipeline/summarize.py`  
...
| 항목 | 값 |
|---|---|
| 경로 | `legacy/fastapi/app/pipeline/summarize.py` |
| 형식 | `python` |
| 전체 줄 수 | 38 |
| 주석 줄 수 | 1 |
| 주요 심볼 수 | 3 |
...
| 선언 | 읽을 때 확인할 점 |
|---|---|
| `summarize_transcript` | 입력, 상태 변경, 반환값과 실패 경로를 확인한다. |
| `_json_content` | 입력, 상태 변경, 반환값과 실패 경로를 확인한다. |
| `make_openai_client` | 입력, 상태 변경, 반환값과 실패 경로를 확인한다. |

## 7. 코드 흐름 상세

### 7.1 `summarize_transcript`
